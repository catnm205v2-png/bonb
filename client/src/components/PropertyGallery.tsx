      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div className="max-w-2xl">
            <h2 className="text-4xl font-serif font-bold text-foreground mb-4">Curated Properties</h2>
            <p className="text-lg text-muted-foreground">
              Explore our portfolio of premium homes. 
              Find your perfect match or see what we've recently sold for our clients.
            </p>
          </div>
          
          <div className="flex gap-2 p-1 bg-background rounded-full border border-border shadow-sm overflow-x-auto w-full md:w-auto">
            <FilterBtn active={filter === "all"} onClick={() => setFilter("all")}>All Properties</FilterBtn>
            <FilterBtn active={filter === "active"} onClick={() => setFilter("active")}>Active</FilterBtn>
            <FilterBtn active={filter === "coming_soon"} onClick={() => setFilter("coming_soon")}>Coming Soon</FilterBtn>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8">
          {filteredProperties.map((property) => (
            <div key={property.id} className="group bg-background rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-xl transition-all duration-300">
              <div className="relative h-72 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <Badge className={
                    property.status === 'sold' ? 'bg-muted text-muted-foreground hover:bg-muted' :
                    property.status === 'coming_soon' ? 'bg-secondary text-secondary-foreground hover:bg-secondary' :
                    'bg-primary text-primary-foreground hover:bg-primary'
                  }>
                    {property.status === 'sold' ? 'SOLD' : property.status === 'coming_soon' ? 'COMING SOON' : 'FOR SALE'}
                  </Badge>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="text-2xl font-serif font-bold text-foreground">{property.price}</div>
                ...
                <div className="flex gap-6 py-4 border-y border-border/50 mb-4">
                  <div className="flex items-center gap-2 text-foreground">
                    ...
                    <span className="font-medium">{property.beds} <span className="text-muted-foreground text-sm font-normal">Beds</span></span>
                  </div>
                  <div className="flex items-center gap-2 text-foreground">
                    <Bath className="h-5 w-5 text-primary" />
                    <span className="font-medium">{property.baths} <span className="text-muted-foreground text-sm font-normal">Baths</span></span>
                  </div>
                  <div className="flex items-center gap-2 text-foreground">
                    <Square className="h-5 w-5 text-primary" />
                    <span className="font-medium">{property.sqft} <span className="text-muted-foreground text-sm font-normal">SqFt</span></span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="outline" className="flex-1 rounded-full border-primary/20 hover:bg-primary/5">
                    Schedule Tour
                  </Button>
                </div>
              </div>
